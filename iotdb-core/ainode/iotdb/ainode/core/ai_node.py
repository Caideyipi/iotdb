...
class AINode:
    ...
    def start(self):
        ...
        if not os.path.exists(system_properties_file):
            ...
            try:
                logger.info("TimechoDB-AINode is registering to TimechoDB cluster...")
                ...
                logger.error(
                    "TimechoDB-AINode failed to register to TimechoDB cluster: {}".format(
                        e
                    )
                )
                raise e
        ...
        if self._rpc_service.exit_code != 0:
            logger.info("TimechoDB-AINode failed to start, please check previous logs.")
            return
    ...
    def _handle_signal(self, signum, frame):
        ...
        logger.info(
            f"TimechoDB-AINode receives {signal_name}, initiating graceful stop..."
        )
        self.stop()
            ...
            if self._rpc_service:
                ...
                for retry in range(30):
                    ...
                    if not self._rpc_service.is_alive():
                        ...
                        break
            logger.info("TimechoDB-AINode has successfully stopped.")
