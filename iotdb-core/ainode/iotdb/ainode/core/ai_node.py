...
1            try:
1                logger.info("TimechoDB-AINode is registering to TimechoDB cluster...")
1                # Check whether the target ConfigNode is enterprise version
1                system_info = (
1                    ClientManager().borrow_config_node_client().getSystemConfiguration()
1                )
1                if (
1                    not system_info.globalConfig
1                    or not system_info.globalConfig.isEnterprise
1                ):
1                    logger.error(
1                        "TimechoDB-AINode registration failed because TimechoDB-AINode can only join TimechoDB cluster."
1                    )
1                    raise MissingConfigError(
1                        "The target ConfigNode is not an enterprise version, "
1                        "please check the configuration of the ConfigNode."
1                    )
1                ainode_id = (
1                    ClientManager()
...
1                logger.error(
1                    "TimechoDB-AINode failed to register to TimechoDB cluster: {}".format(
1                        e
1                    )
1                )
1                raise e
...
1            try:
1                logger.info("TimechoDB-AINode is restarting...")
...
1                )
1            except Exception as e:
1                logger.error("TimechoDB-AINode failed to restart: {}".format(e))
1                raise e
1
...
1            logger.info("TimechoDB-AINode failed to start, please check previous logs.")
1            return
1
1        logger.info("TimechoDB-AINode has successfully started.")
1
...
1
1        logger.info(
1            f"TimechoDB-AINode receives {signal_name}, initiating graceful stop..."
1        )
1        self.stop()
1
1    def stop(self):
...
1            logger.info("TimechoDB-AINode has successfully stopped.")
1