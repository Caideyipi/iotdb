...
1        return False
1
1    def getSystemConfiguration(self) -> TSystemConfigurationResp:
1        for _ in range(0, self._RETRY_NUM):
1            try:
1                resp = self._client.getSystemConfiguration()
1                if not self._update_config_node_leader(resp.status):
1                    verify_success(
1                        resp.status,
1                        "An error occurs when calling getSystemConfiguration()",
1                    )
1                    return resp
1            except TTransport.TException:
1                logger.warning(
1                    "Failed to connect to ConfigNode {} from AINode when executing getSystemConfiguration()",
1                    self._config_leader,
1                )
1                self._config_leader = None
