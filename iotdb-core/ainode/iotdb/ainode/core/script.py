...
1        logger.info("TimechoDB AINode has successfully removed.")
...
1            logger.info("TimechoDB AINode is starting...")
1            ai_node = AINode()
1            ai_node.start()
1        except Exception as e:
1            logger.error("Start TimechoDB AINode failed, because of: {}".format(e))
1            sys.exit(1)
1    # TODO: remove the following function, and add a destroy script
1    elif command == "remove":
1        try:
1            logger.info("Removing TimechoDB AINode...")
1            remove_ainode(arguments)
1        except Exception as e:
1            logger.error("Remove TimechoDB AINode failed, because of: {}".format(e))
1            sys.exit(1)
1    else:
