...
1
1        # activation
1        self._ain_activated = threading.Event()
1
...
1
1    def is_activated(self) -> bool:
1        return self._ain_activated.is_set()
1
1    def set_activated(self, is_activated: bool) -> None:
1        if is_activated:
1            if not self._ain_activated.is_set():
1                logger.info("TimechoDB-AINode is activated.")
1            self._ain_activated.set()
1        else:
1            if self._ain_activated.is_set():
1                logger.info("TimechoDB-AINode is deactivated.")
1            self._ain_activated.clear()
1
...
1                )
1
1            if "ain_internal_ssl_enabled" in config_keys:
1                self._config.set_ain_internal_ssl_enabled(
1                    int(file_configs["ain_internal_ssl_enabled"])
1                )
1
