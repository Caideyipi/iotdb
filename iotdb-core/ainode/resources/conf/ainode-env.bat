...
for %%i in (*.whl *.tar.gz) do (
    echo %%i | findstr "ainode" >nul && (
        ...
        %ain_interpreter_dir% -m pip install %%i %ain_force_reinstall% -i https://pypi.tuna.tsinghua.edu.cn/simple --no-warn-script-location %ain_no_dependencies% --find-links https://download.pytorch.org/whl/torch_stable.html
    ) || (
