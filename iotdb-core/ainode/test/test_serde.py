...
import numpy as np
import pandas as pd
from iotdb.ainode.serde import convert_to_df
...
column_name_index = {
    "root.wt1.windSpeed": 0,
    "root.wt1.temperature": 1,
    "root.wt1.hardware": 2,
    "root.wt1.status": 3,
    "root.wt1.altitude": 4,
    "root.wt1.angle": 5,
...
def test_simple_query():
    ...
    df_output = convert_to_df(
        column_names, data_type_list, column_name_index, simple_binary
    )
...
def test_with_null_query():
    ...
    timestamps = np.arange(data_nums, dtype="int64")
    data[ts_path_lst[0]] = [
        0.049136366695165634,
        0.4814307689666748,
        None,
        None,
        0.3209269344806671,
        0.3143160343170166,
        0.030140314251184464,
        None,
        0.027547180652618408,
        0.2688102126121521,
        0.9617357850074768,
        None,
        0.30917245149612427,
    ...
    df_output = convert_to_df(
        column_names, data_type_list, column_name_index, binary_with_null
    )
    df_output = df_output[df_input.columns.tolist()]
    assert_frame_equal(df_input, df_output)
