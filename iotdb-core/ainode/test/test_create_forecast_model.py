...
from iotdb.ainode.constant import OptionsKey
from iotdb.ainode.exception import (
    BadConfigValueError,
    MissingOptionError,
    UnsupportedError,
)
from iotdb.ainode.parser import ForecastTaskOptions, parse_task_options
from iotdb.ainode.util import pack_input_dict


def test_create_forecast_model():
    d_forecast_task_options = ForecastTaskOptions(
        {
            OptionsKey.MODEL_TYPE.name(): ForecastModelType.DLINEAR.value,
            OptionsKey.AUTO_TUNING.name(): "False",
        }
    )
    d_configs = {
        HyperparameterName.KERNEL_SIZE.name(): "25",
        HyperparameterName.USE_GPU.name(): "False",
    }
    ...
    model_configs[OptionsKey.PREDICT_LENGTH.value] = (
        d_forecast_task_options.predict_length
    )
    model = create_forecast_model(d_forecast_task_options.model_type, model_configs)
    sample_input = torch.randn(
        1,
        model_configs[OptionsKey.INPUT_LENGTH.value],
    ...
    n_forecast_task_options = ForecastTaskOptions(
        {OptionsKey.MODEL_TYPE.name(): ForecastModelType.NBEATS.value}
    )
    n_configs = {
        HyperparameterName.KERNEL_SIZE.name(): "25",
        HyperparameterName.D_MODEL.name(): "64",
    }
    ...
    model_configs[OptionsKey.PREDICT_LENGTH.name()] = (
        n_forecast_task_options.predict_length
    )
    ...
    assert (
        "kernel_size" not in model_configs
    )  # config kernel_size not belongs to nbeats model
...
def test_bad_config_model1():
    try:
        d_forecast_task_options = ForecastTaskOptions(
            {
                OptionsKey.MODEL_TYPE.name(): "dlinear_dummy",
                OptionsKey.AUTO_TUNING.name(): "False",
            }
        )
        d_configs = {
        ...
        assert (
            e.message == "model_type dlinear_dummy is not supported in current version"
        )
...
def test_bad_config_model2():
    try:
        d_forecast_task_options = ForecastTaskOptions(
            {
                OptionsKey.MODEL_TYPE.name(): "dlinear",
                OptionsKey.AUTO_TUNING.name(): "False",
            }
        )
        d_configs = {
            HyperparameterName.KERNEL_SIZE.name(): "-1",
            HyperparameterName.USE_GPU.name(): "False",
        }
        ...
        assert (
            e.message
            == "Bad value [-1] for config kernel_size. Expect value between 1 and 10000000000.0, "
            "got -1 instead."
        )
