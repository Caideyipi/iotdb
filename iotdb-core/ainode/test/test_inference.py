...
from iotdb.ainode.inference import inference_with_registered_model, process_data, create_built_in_model, \
...
def test_process_data_with_bool():
    data = pd.DataFrame({'a': [1, 2, 3, 4, 5], 'b': [
        1.0, 2.1, 3.3, 4.2, 5.5], 'c': [True, False, True, True, True]})
    time_stamp = pd.DataFrame({'time': [1, 2, 3, 4, 5]})
    type_list = ['INT32', 'FLOAT', 'BOOLEAN']
    column_name_list = ['a', 'b', 'c']
    tensor_data = torch.tensor([[[1, 1.0, 1], [2, 2.1, 0], [3, 3.3, 1], [
        4, 4.2, 1], [5, 5.5, 1]]], dtype=torch.float64)
...
def test_process_data_with_text():
    data = pd.DataFrame({'a': [1, 2, 3, 4, 5], 'b': [
        1.0, 2.1, 3.3, 4.2, 5.5], 'c': ['a', 'b', 'c', 'd', 'e']})
    time_stamp = pd.DataFrame({'time': [1, 2, 3, 4, 5]})
    type_list = ['INT32', 'FLOAT', 'TEXT']
    column_name_list = ['a', 'b', 'c']
    tensor_data = torch.tensor([[[1, 1.0, 0], [2, 2.1, 0], [3, 3.3, 0], [
        4, 4.2, 0], [5, 5.5, 0]]], dtype=torch.float64)
    full_data = (data, time_stamp, type_list, column_name_list)
    dataset, dataset_length = process_data(full_data)
    assert dataset.shape == (1, 5, 3)
    assert dataset_length == 5
...
def test_process_data_with_none():
    data = pd.DataFrame({'a': [1, 2, 3, 4, 5], 'b': [
        1.0, 2.1, 3.3, 4.2, 5.5], 'c': [None, 1, None, None, None]})
    time_stamp = pd.DataFrame({'time': [1, 2, 3, 4, 5]})
    type_list = ['INT32', 'FLOAT', 'INT32']
    column_name_list = ['a', 'b', 'c']
    tensor_data = torch.tensor([[[1, 1.0, 0], [2, 2.1, 1], [3, 3.3, 0], [
        4, 4.2, 0], [5, 5.5, 0]]], dtype=torch.float64)
    full_data = (data, time_stamp, type_list, column_name_list)
    dataset, dataset_length = process_data(full_data)
    assert dataset.shape == (1, 5, 3)
