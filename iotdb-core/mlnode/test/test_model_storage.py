...
from iotdb.thrift.mlnode.ttypes import TConfigs
from iotdb.mlnode.constant import DEFAULT_MODEL_FILE_NAME, DEFAULT_CONFIG_FILE_NAME
...
def test_save_model():
    ...
    assert os.path.exists(
        os.path.join(descriptor.get_config().get_mln_models_dir(), f'{model_id}', f'{trial_id}.pt'))
...
def test_load_not_exist_model():
    ...
    try:
        ...
        assert e.message == ModelNotExistError(
            os.path.join(os.getcwd(), descriptor.get_config().get_mln_models_dir(),
                         model_id, f'{trial_id}.pt')).message


storage_path = './data/mlnode/models'
