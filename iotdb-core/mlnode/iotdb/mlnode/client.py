...
from iotdb.thrift.confignode.ttypes import (TUpdateModelInfoReq,
                                            TUpdateModelStateReq, TMLNodeRemoveReq, TNodeVersionInfo,
                                            TMLNodeRegisterReq, TMLNodeRestartReq)
...
class ClientManager(object):
    def __init__(self):
        ...
        self.__config_node_endpoint = descriptor.get_config().get_mln_target_config_node_list()
...
class ConfigNodeClient(object):
    ...
    def node_register(self, cluster_name: str, configuration: TMLNodeConfiguration,
                      version_info: TNodeVersionInfo) -> int:
        req = TMLNodeRegisterReq(
            clusterName=cluster_name,
            mlNodeConfiguration=configuration,
            versionInfo=version_info
        )

        for _ in range(0, self.__RETRY_NUM):
            try:
                resp = self.__client.registerMLNode(req)
                if resp.status.code != TSStatusCode.SUCCESS_STATUS.get_status_code():
                    raise RuntimeError(f"Failed to register MLNode to ConfigNode, status: {resp.status}")
                self.__config_nodes = resp.configNodeList
                return resp.mlNodeId
            except TTransport.TException:
                logger.warning("Failed to connect to ConfigNode {} from MLNode when executing node_register()",
                               self.__config_leader)
                self.__config_leader = None
            self.__wait_and_reconnect()
    ...
    def node_restart(self, cluster_name: str, configuration: TMLNodeConfiguration,
                     version_info: TNodeVersionInfo) -> None:
        req = TMLNodeRestartReq(
            clusterName=cluster_name,
            mlNodeConfiguration=configuration,
            versionInfo=version_info
        )
        ...
        for _ in range(0, self.__RETRY_NUM):
            try:
                resp = self.__client.restartMLNode(req)
                if resp.status.code != TSStatusCode.SUCCESS_STATUS.get_status_code():
                    raise RuntimeError(f"Failed to restart MLNode to ConfigNode, status: {resp.status}")
                self.__config_nodes = resp.configNodeList
                return resp.status
            except TTransport.TException:
                logger.warning("Failed to connect to ConfigNode {} from MLNode when executing node_restart()",
                               self.__config_leader)
                self.__config_leader = None
            self.__wait_and_reconnect()
    ...
    def node_remove(self, location: TMLNodeLocation):
        req = TMLNodeRemoveReq(
            mlNodeLocation=location
        )
        for _ in range(0, self.__RETRY_NUM):
            try:
                status = self.__client.removeMLNode(req)
                if status.code != TSStatusCode.SUCCESS_STATUS.get_status_code():
                    raise RuntimeError(f"Failed to remove MLNode to ConfigNode, status: {status}")
                return status
            except TTransport.TException:
                logger.warning("Failed to connect to ConfigNode {} from MLNode when executing node_remove()",
                               self.__config_leader)
                self.__config_leader = None
            self.__wait_and_reconnect()
        raise TException(self.__MSG_RECONNECTION_FAIL)
