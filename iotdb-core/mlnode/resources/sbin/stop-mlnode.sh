...
if  type lsof > /dev/null 2>&1 ; then
  echo $(lsof -t -i:"${mln_inference_rpc_port}" -sTCP:LISTEN)
  PID=$(lsof -t -i:"${mln_inference_rpc_port}" -sTCP:LISTEN)
elif type netstat > /dev/null 2>&1 ; then
  PID=$(netstat -anp 2>/dev/null | grep ":${mln_inference_rpc_port} " | grep ' LISTEN ' | awk '{print $NF}' | sed "s|/.*||g" )
else
  echo ""
